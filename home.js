...
const Home = {
  render(){
    ...
    document.getElementById('sumItems').textContent = soon>0 ? `<i class="ph ph-warning"></i> ${soon} 項即將到期` : `共 ${State.items.length} 項追蹤中`;
    ...
    if(todos.length===0){
      list.innerHTML = `<div class="empty"><span class="big"><i class="ph ph-sun"></i></span>今天還沒有安排代辦事項<br><span style="font-size:12px;" onclick="App.go('calendar')">前往行事曆新增 →</span></div>`;
      return;
    }
